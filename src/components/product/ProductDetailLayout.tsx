...
const ProductDetailLayout: React.FC<ProductDetailLayoutProps> = ({
  ...
  return (
    <div className="flex flex-col min-h-0 bg-white overscroll-none pb-20" ref={refs.contentRef}>
      ...
      {/* Main Content Sections - Only render tab-specific content */}
      ...
      {/* Sticky Components */}
      {/* Conditionally render StickyCheckoutBar only if activeSection is 'overview' */}
      {state.activeSection === 'overview' && (
        <StickyCheckoutBar
          ...
          hideCheckoutBar={false} // This prop might need adjustment based on StickyCheckoutBar's internal logic
        />
      )}
    </div>
  );
